...
public class SocialService {
    ...
    /**
     * Raccoglie i metadati per creare e salvare un nuovo Post di un Utente sulla piattaforma.
     *
    ...
    /**
     * Interroga JPA per ottenere in ordine cronologico inverso la galleria generale dei post (Bacheca).
     *
    ...
    public org.springframework.data.domain.Page<Post> getFeed(int page, int size) {
        return postRepository.findAllByOrderByCreationDateDesc(org.springframework.data.domain.PageRequest.of(page, size));
    }

    /**
     * Gestisce l'aggiunta di un blocco di testo commento sotto un certo post autorizzato da un utente valido.
     *
    ...
    /**
     * Preforma l'eliminazione profonda di un post, assicurandosi nel while logico che
     * solo l'utente che lo ha creato possa invocarne la distruzione per sicurezza orizzontale.
     *
    ...
    public void deletePost(@NonNull Long postId, @NonNull Long userId) {
        ...
        if (!post.getAuthor().getId().equals(userId)) {
            throw new org.springframework.security.access.AccessDeniedException("Non sei autorizzato a eliminare questo post");
        }
